...
Mean activity for `r "lemon"` in the control group was `r "mean(lemon_data_control$activity)"` (s / min) compared to the OA treatment group, which was `r "mean(lemon_data_treatment$activity)"` (s / min). The difference between control and OA treatment means was `r "mean(lemon_data_treatment$activity)-mean(lemon_data_control$activity)"` (s / min) (95% CI: `r "t_test_acantho$conf.int[1]"` to `r "t_test_acantho$conf.int[2]"`). 
...
After the permutation test of all the species, we find out that the p-value is still larger than 'r  p_value = 0.05;p_value', which means we accept the null hypothesis which indicate that there is no significant differences between the control group and the treatment group. We can get to the conclusion that Acidification did not play an obvious role among different kind of fishes.
